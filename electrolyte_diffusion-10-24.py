...
#     #ax.rcParams.update({'font.size': 18})
    ...
    if(it == 0):
      cn_plate = plt.figure(it, frameon=True, figsize=[8.0, 10.0],dpi = 300, constrained_layout= True)
      ax=plt.gca()   
      ax.set_facecolor('#74ccf4')
      ...
      plt.title("Diffusion of NaCl in water \n t = %5.2f hr/ %5.2f hr" %(time, Ttotal_hr), fontsize=22)
...
while(it<=Nt):
    if(it ==0):
        ...
        time = it*dt/3600
        Ttotal_hr = Ttotal/3600
        phi_plate = plt.figure(it, frameon=True, figsize=[8.0, 10.0],dpi = 300, constrained_layout= True)
        plt.xlim(0, Lx)
        plt.ylim(0, 5)
        ...
        plt.title("Diffusion of NaCl in water \n t = %5.2f s/ %5.2f s" %(time, Ttotal_hr), fontsize=22)
    #plate.patch.set_alpha(0.0)
    #plate.patch.set_alpha(1.0)
