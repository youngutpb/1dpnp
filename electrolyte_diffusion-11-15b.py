...
iplot = 0
iframe = 0
norm_Ex_max = np.amax(Ex,axis = 1)
print("it_Ex_max= ", norm_Ex_max)
norm = clr.Normalize()
norm_Ex = np.linspace(0, 30, 3000)
norm.autoscale(np.absolute(Ex[:,:]))
cm = plt.cm.Reds
...
# plt.quiver(znum, rnum, z_vector, r_vector, color='saddlebrown', pivot='middle')
        ...
        vecEx_plot = Ex[it,::num_skip]
        vecEx_plot_mag = np.absolute(Ex[it,::num_skip])
        vecEx_plot_dir = Ex[it,::num_skip]/23
        
        
        # find_size = np.size(Ex[it,::])
        # vecEx_plot = np.array((find_size))
        # vecEx_plot = Ex[it,::]
        # vecEx_plot_mag = np.absolute(Ex[it,:])
        # vecEx_plot_dir = Ex[it,::]/vecEx_plot_mag
        ...
        yplot = np.linspace(-1.0, 1.0, num_vecs)
        ...
        vecEx_plot_dir, Ey = np.meshgrid(vecEx_plot_dir, Ey)
        #vecEx_plot_mag_stack = np.hstack(np.repeat(vecEx_plot_mag, num_vecs))
        #vecEx_plot_mag, yplot = np.meshgrid(vecEx_plot_mag, yplot)
        ...
        #plt.quiver(xplot,yplot,vecEx_plot_dir, Ey , 23,  color=cm(np.absolute(Ex[it,:])), pivot='middle')
        plt.quiver(xplot,yplot,vecEx_plot_dir, Ey, color='saddlebrown', pivot='middle')

        #plt.quiver(xplot,yplot,vecEx_plot_dir, Ey, color=cm(np.absolute(Ex[it,::num_skip])), pivot='middle')
        #plt.colorbar(sm)
    #
