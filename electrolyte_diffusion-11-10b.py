...
Trun = 1.0 # length of "experiment" in hours
dt = 3600/20
print("dt=", dt)
...
# plt.quiver(znum, rnum, z_vector, r_vector, color='saddlebrown', pivot='middle')
        ...
        #xplot = np.zeros(x[::4])
        Narrows = 1
        yplot = np.zeros(np.size(x[::Narrows]))
        yplot = Ex[it,::Narrows]
        y = np.zeros(Nx+1)
        #y = np.linspace(0,1.0, Nx+1)
        Ey = 0.0*np.linspace(0,1.0, Nx+1)
        ...
        #x,y = np.meshgrid(x,y)
        Excurrent = np.zeros(np.size(Ex[it,::Narrows]))
        
        Excurrent = Ex[it,::Narrows]
        Excurrent, Ey = np.meshgrid(Excurrent, Ey[::Narrows])
        # fig, ax = plt.subplots()
        # q = ax.quiver(x, y, Excurrent, Ey)
        
        plt.quiver(x[::Narrows],yplot,Excurrent, Ey, Ex_norm, color="firebrick", pivot='middle')
