...
C_analytic = phi_right/Lx - 0.5*rho_eo*Lx - phi_left/Lx
for i in np.arange(0, Nx+1):
    x[i] = i*dx
    phi_plot_analytic[i] = 0.5*rho[0,i]*x[i]**2 + C_analytic*x[i] + phi_left
...
play_done_sound = subprocess.run(["play", finished_sound])
